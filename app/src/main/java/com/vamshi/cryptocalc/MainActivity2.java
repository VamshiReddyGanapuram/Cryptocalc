...
public class MainActivity2 extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    ...
    private void calculateParams(InputData inputData) {
        ...
        outputData.BuyingFee=(float) Math.floor((inputData.RealtimePrice*inputData.BuyingFee)/100);
        outputData.CoinsPurchasedinclBF=(float) Math.floor(inputData.Investment/(inputData.RealtimePrice+outputData.BuyingFee));
        outputData.RiskAmount=(float) Math.floor((inputData.Investment*(inputData.RiskPercent/100)));
        outputData.WorseRiskAmount=(float) Math.floor(inputData.Investment-outputData.RiskAmount);
        outputData.SellingFeeForRisk=(float) Math.floor((outputData.WorseRiskAmount*inputData.SellingFee)/100);
        outputData.TotalAmountwithSF=outputData.WorseRiskAmount-outputData.SellingFeeForRisk;
        outputData.ProfitAmount=(float) Math.floor((inputData.SellRate*outputData.CoinsPurchasedinclBF)-inputData.Investment);
        outputData.ProfitAmountwithSF=(float) Math.floor(outputData.ProfitAmount-(((inputData.SellRate*outputData.CoinsPurchasedinclBF)*inputData.SellingFee)/100));
